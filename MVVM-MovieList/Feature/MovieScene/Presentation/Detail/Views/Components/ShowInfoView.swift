...
class ShowInfoView: UIView {
    ...
    func setParameters(for movie: Movie) {
        let scheduleText = "Schedule : \(movie.days.map({$0.rawValue}).joined(separator: ", ")) at \(movie.time.rawValue) (\(movie.averageRuntime) min)"
        ...
        ratingView.setRating(with: movie.rating)
    }
